...
yagidesign.ReflectorLength = GA_SLL_Cons_Ant(1);
yagidesign.DirectorLength = GA_SLL_Cons_Ant(3:6);
yagidesign.ReflectorSpacing = GA_SLL_Cons_Ant(7);
yagidesign.DirectorSpacing = GA_SLL_Cons_Ant(8:11);
...
disp(['GA-Optimised Antenna Gain (dBi): ', num2str(gain)]);
...
legend('GA-Optimised Design', 'Conventional Design', 'Location', 'northoutside');
rlim([-38 20]); 
thetaticks(0:30:360);
hold off;
...
xlabel('Theta (degree)');
ylabel('Gain (dBi)');
title('Cartesian Radiation Patterns of GA and Conventional Antennas');
legend('GA-Optimised Design', 'Conventional Design');
