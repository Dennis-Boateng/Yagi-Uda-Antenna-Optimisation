...
yagidesign_ini = yagiUda;
yagidesign_ini.Exciter = d;
yagidesign_ini.NumDirectors = numDirectors;
yagidesign_ini.ReflectorLength = initialAntenna(1);
yagidesign_ini.DirectorLength = initialAntenna(3:6);
yagidesign_ini.ReflectorSpacing = initialAntenna(7);
yagidesign_ini.DirectorSpacing = initialAntenna(8:11);
...
% Gain Calculations
gain_ini = max(pattern(yagidesign_ini, frequency_Hz, 0, 0:1:360));
...
lobeInfo_ini = findLobes(polarpattern(pattern(yagidesign_ini, frequency_Hz, 0, 0:1:360)));
sll_ini = -lobeInfo_ini.SLL;
...
disp(['Conventional Antenna Gain (dBi): ', num2str(gain_ini)]);
disp(['Conventional Antenna SLL (dB): ', num2str(sll_ini)]);
disp(' ');
...
figure; pattern(yagidesign_ini, frequency_Hz); title('Polar Radiation Pattern');
...
Eplane_gain_ini = pattern(yagidesign_ini, frequency_Hz, 0, theta_deg);
Eplane_gain_SA = pattern(yagidesign, frequency_Hz, 0, theta_deg);
Eplane_gain_GA = pattern(yagidesign_2, frequency_Hz, 0, theta_deg);
...
% E-plane Plots
figure;
polarplot(theta_rad, Eplane_gain_ini, '--k', 'LineWidth', 2); hold on;
polarplot(theta_rad, Eplane_gain_SA, ':b', 'LineWidth', 2); 
polarplot(theta_rad, Eplane_gain_GA, '-r', 'LineWidth', 2);
title('E-plane Polar Radiation Pattern (Azimuth = 0°)');
legend('Conventional Design', 'SA-Optimised Design', 'GA-Optimised Design', 'Location', 'northoutside');
rlim([-40 20]); 
thetaticks(0:30:360);
hold off;

% H-plane gain patterns for both antennas
Hplane_gain_ini = pattern(yagidesign_ini, frequency_Hz, 90, theta_deg);
Hplane_gain_SA = pattern(yagidesign, frequency_Hz, 90, theta_deg);
Hplane_gain_GA = pattern(yagidesign_2, frequency_Hz, 90, theta_deg);

% Polar plot Creation
figure;
polarplot(theta_rad, Hplane_gain_ini, '--k', 'LineWidth', 2); hold on;
polarplot(theta_rad, Hplane_gain_SA, ':b', 'LineWidth', 2); 
polarplot(theta_rad, Hplane_gain_GA, '-r', 'LineWidth', 2);
...
figure; yagidesign_ini.Tilt = 90; yagidesign_ini.TiltAxis = [0 1 0]; show(yagidesign_ini);
...
% 2D Polar Plots
theta = -180:1:180;
gain_dBi_ini = pattern(yagidesign_ini, frequency_Hz, 0, theta);
gain_dBi_SA = pattern(yagidesign, frequency_Hz, 0, theta);
gain_dBi_GA = pattern(yagidesign_2, frequency_Hz, 0, theta);

figure;
hold on; 
plot(theta, gain_dBi_ini, '--k', 'LineWidth', 2); 
plot(theta, gain_dBi_SA, ':b', 'LineWidth', 2); 
plot(theta, gain_dBi_GA, '-r', 'LineWidth', 2); 
grid on;
xlabel('Theta (degree)');
ylabel('Gain (dBi)');
title('Cartesian Radiation Patterns of All Designs');
legend('Conventional Design', 'SA-Optimised Design', 'GA-Optimised Design');
